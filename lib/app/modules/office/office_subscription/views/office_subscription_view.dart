...
class _UpgradeCard extends StatefulWidget {
  final OfficeSubscriptionController ctrl;
  const _UpgradeCard({required this.ctrl});
...
class _UpgradeCardState extends State<_UpgradeCard> {
  ...
  void initState() {
    ...
    _vehicleCtrl = TextEditingController(
        text: widget.ctrl.vehicleInput.value);
  ...
  String _fmt(double v) =>
      '₹${v.toStringAsFixed(2).replaceAllMapped(RegExp(r'(\d)(?=(\d{2})+\d\.)'), (m) => '${m[1]},')}';

  @override
  Widget build(BuildContext context) {
    return Obx(() {
      final ctrl    = widget.ctrl;
      final plans   = ctrl.plans;
      final sent    = ctrl.upgradeSent.value;
      final loading = ctrl.upgradeState.value == ViewState.loading;

      return _Card(
        title: 'Upgrade Your Plan',
        titleIcon: Icons.arrow_circle_up_outlined,
        child: sent
            ? _sentState(ctrl)
            : Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  ...
                  if (plans.isEmpty)
                    ...
                    ...plans.map((plan) {
                      ...
                      final selected = ctrl.selectedPlanId.value == id;
                      final price    = (plan['pricePerVehicle'] as num? ?? 0).toDouble();
                      final minV     = (plan['minVehicles'] as num? ?? 0).toInt();
                      final maxV     = (plan['maxVehicles'] as num? ?? -1).toInt();

                      return GestureDetector(
                        onTap: () => ctrl.selectPlan(id),
                        child: Container(
                          ...
                          child: Row(
                            children: [
                              ...
                              Text(
                                '₹${price.toStringAsFixed(0)}/v/month',
                                style: AppTextStyles.bodyMedium.copyWith(
                                    color: AppColors.navy,
                                    fontWeight: FontWeight.w700),
                              ),
                  ...
                  // Config section (shown when plan selected)
                  if (ctrl.selectedPlanId.value != null) ...[
                    const SizedBox(height: 8),
                    const Divider(),
                    const SizedBox(height: 8),
                    Text('Configure your plan',
                        style: AppTextStyles.bodyMedium
                    ...
                    TextFormField(
                      ...
                      onChanged: ctrl.setVehicleInput,
                    ...
                    GridView.count(
                      ...
                      mainAxisSpacing: 8,
                      childAspectRatio: 3,
                      children: OfficeSubscriptionController.cycles
                          .map((c) {
                        final val      = c['value']!;
                        final label    = c['label']!;
                        final selected =
                            ctrl.billingCycle.value == val;
                        return GestureDetector(
                          onTap: () =>
                              ctrl.billingCycle.value = val,
                          child: Container(
                      ...
                      Container(
                        ...
                        child: Column(
                          children: [
                            ...
                            const SizedBox(height: 8),
                            _EstRow(
                              '${ctrl.vehicleInput.value} vehicles × ₹... × ${ctrl.estimate!['months']} month(s)',
                              _fmt(ctrl.estimate!['base'] as double),
                            ),
                            _EstRow('GST 18%',
                                _fmt(ctrl.estimate!['gst'] as double)),
                            const Divider(height: 12),
                            _EstRow(
                              'Total',
                              _fmt(ctrl.estimate!['total'] as double),
                              bold: true,
                            ),
                            const SizedBox(height: 4),
                            Text(
                              'GST paid is claimable as ITC for GST-registered businesses.',
                              style: AppTextStyles.caption.copyWith(
                                  color: AppColors.mutedText,
                                  fontSize: 10),
                            ),
                    ...
                    SizedBox(
                      ...
                      child: ElevatedButton(
                        onPressed: loading
                            ...
                            : () async {
                                final ok =
                                    await ctrl.submitUpgradeRequest();
                        ...
                        child: loading
                            ? const SizedBox(
                                ...
                                child: CircularProgressIndicator(
                                    strokeWidth: 2,
                                    color: Colors.white))
                            : const Text('Send Upgrade Request',
                                style: TextStyle(
                                    fontWeight: FontWeight.w600)),
                      ),
                    ),
                    ...
                    Center(
                      child: Text(
                        'We\'ll contact you to confirm payment and activate.',
                        style: AppTextStyles.caption
                            .copyWith(color: AppColors.mutedText),
  ...
  Widget _sentState(OfficeSubscriptionController ctrl) {
    return Column(
      children: [
        ...
        const SizedBox(height: 6),
        Text(
          'We\'ve received your upgrade request. Our team will contact you shortly to confirm and activate your plan.',
          style: AppTextStyles.caption
              .copyWith(color: AppColors.mutedText),
          textAlign: TextAlign.center,
        ),
        const SizedBox(height: 16),
        TextButton(
          onPressed: ctrl.resetUpgradeForm,
...
class _EstRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final style = AppTextStyles.caption.copyWith(
      ...
      color: bold ? AppColors.bodyText : AppColors.mutedText,
    );
    return Padding(
