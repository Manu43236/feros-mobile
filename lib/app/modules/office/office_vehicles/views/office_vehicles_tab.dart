...
// Reuse supervisor controller (same API, same logic) and detail view
import '../../../supervisor/supervisor_vehicles/controllers/supervisor_vehicles_controller.dart';
import '../../../supervisor/supervisor_vehicles/bindings/supervisor_vehicle_detail_binding.dart';
import '../../../supervisor/supervisor_vehicles/views/supervisor_vehicle_detail_view.dart';
import 'office_vehicle_form_view.dart';

class OfficeVehiclesTab extends StatelessWidget {
  const OfficeVehiclesTab({super.key});

  @override
  Widget build(BuildContext context) {
    final controller = Get.find<SupervisorVehiclesController>();

    final role = Get.find<AuthService>().user?.role ?? '';
    final canManage = role == 'ADMIN' || role == 'OFFICE_STAFF';
      ...
      floatingActionButton: canManage
          ? FloatingActionButton.extended(
              ...
              onPressed: () async {
                ...
                if (added == true) controller.fetchVehicles();
              },
...
class _VehicleCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      onTap: () => Get.to(
        () => const SupervisorVehicleDetailView(isOffice: true),
            ...
            Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  if (_hasExpiry(rcExp, insExp, permitExp, fitnessExp, pucExp)) ...[
  ...
  static (Color, Color) _statusColors(String type) {
    switch (type) {
      ...
      case 'IN_SERVICE':  return (const Color(0xFF7C3AED), const Color(0xFFF5F3FF));
...
class _ExpiryChip extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final (bg, fg, icon) = switch (status) {
      _Expiry.expired  => (const Color(0xFFFEE2E2), const Color(0xFFDC2626), Icons.warning_amber_rounded),
      _Expiry.critical => (const Color(0xFFFFF7ED), const Color(0xFFEA580C), Icons.warning_amber_rounded),
      _Expiry.warning  => (const Color(0xFFFFFBEB), const Color(0xFFD97706), Icons.schedule_outlined),
      _Expiry.ok       => (const Color(0xFFF0FDF4), const Color(0xFF16A34A), Icons.check_circle_outline),
      _Expiry.none     => (Colors.transparent, AppColors.mutedText, Icons.circle_outlined),
    };

    final days = _daysUntil(date);
    final text = status == _Expiry.expired ? '$label expired' : '$label ${days}d';
