...
class _OfficeClientFormViewState extends State<OfficeClientFormView> {
  ...
  Future<void> _prefillFromClient(
    ...
    if (typeId != null)
      _selectedType = types.firstWhereOrNull((t) => t['id'] == typeId);
    if (stateId != null)
      selState = states.firstWhereOrNull((s) => s['id'] == stateId);
    if (termsId != null)
      _selectedTerms = terms.firstWhereOrNull((t) => t['id'] == termsId);
  ...
  Future<void> _submit() async {
    ...
    if (name.isEmpty) {
      FerosSnackbar.error('Client name is required');
      return;
    }
    if (_selectedType == null) {
      FerosSnackbar.error('Select a client type');
      return;
    }
    ...
    try {
      ...
      if (_gstinCtrl.text.trim().isNotEmpty)
        body['gstin'] = _gstinCtrl.text.trim();
      if (_panCtrl.text.trim().isNotEmpty)
        body['panNumber'] = _panCtrl.text.trim();
      if (_cpNameCtrl.text.trim().isNotEmpty)
        body['contactPersonName'] = _cpNameCtrl.text.trim();
      if (_cpPhoneCtrl.text.trim().isNotEmpty)
        body['contactPersonPhone'] = _cpPhoneCtrl.text.trim();
      if (_cpEmailCtrl.text.trim().isNotEmpty)
        body['contactPersonEmail'] = _cpEmailCtrl.text.trim();
      if (_selectedTerms != null)
        body['paymentTermsId'] = _selectedTerms!['id'];
  ...
  Future<void> _toggleStatus() async {
    ...
    try {
      ...
      FerosSnackbar.success(
        _isActive ? 'Client activated' : 'Client deactivated',
      );
    } catch (e) {
      FerosSnackbar.error(e.toString());
    } finally {
                    ...
                    children: [
                      _isLoadingTerms
                          ...
                          : FerosSelectField<Map<String, dynamic>>(
                              ...
                              itemLabel: (t) => t['name'] as String? ?? '',
                              selectedDisplay:
                                  _selectedTerms?['name'] as String?,
                              onSelected: (t) =>
                                  setState(() => _selectedTerms = t),
                            ),
                      const SizedBox(height: 12),
                      _field(
                        'Credit Limit (₹)',
